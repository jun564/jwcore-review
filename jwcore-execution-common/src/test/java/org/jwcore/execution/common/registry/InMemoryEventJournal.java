...
public final class InMemoryEventJournal implements IEventJournal {
    ...
    @Override public long append(final EventEnvelope envelope) { final long next = sequence.incrementAndGet(); final EventEnvelope canonical = withSequence(envelope, next); events.add(canonical); consumers.forEach(c -> c.accept(canonical)); return next; }
    ...
    @Override public List<EventEnvelope> readAfterSequence(final long since) { return events.stream().filter(e -> e.sequenceNumber() > since).sorted(Comparator.comparingLong(EventEnvelope::sequenceNumber)).toList(); }
    ...
    private static EventEnvelope withSequence(final EventEnvelope envelope, final long seq) {
        return new EventEnvelope(envelope.eventId(), envelope.eventType(), envelope.brokerOrderId(), envelope.localIntentId(), envelope.canonicalId(), envelope.idempotencyKey(), seq, envelope.timestampEvent(), envelope.payloadVersion(), envelope.payload(), envelope.sourceProcessId(), envelope.correlationId());
    }
}
