...
final class InMemoryEventJournal implements IEventJournal {
    ...
    @Override public long append(final EventEnvelope envelope) { final long next = sequence.incrementAndGet(); final EventEnvelope canonical = withSequence(envelope, next); events.add(canonical); tails.forEach(t -> t.accept(canonical)); return next; }
